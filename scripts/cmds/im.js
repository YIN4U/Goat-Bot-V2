...
module.exports = {
  ...
  onStart: async function ({ api, event, args }) {
    ...
    try {
      ...
      const response = await axios.get(`https://www.samirxpikachu.run.place/imagine?prompt=${encodeURIComponent(translatedPrompt)}`, { responseType: 'arraybuffer' });
      const data = Buffer.from(response.data, "utf8");
      const filePath = __dirname + '/tmp/emi.png';
